...
**Peanut Uniform Placement in Kung Pao Chicken — Methodology & Computation**

---

> 宫保鸡丁中的花生应随机、均匀地分布于圆形盘面之上。本文档给出圆内均匀随机点生成的数学原理、两种实现方法、示例计算与 Python 演示代码。

---

## 📖 目录

- [一、基础原理：圆的参数](#-一基础原理圆的参数)
- [二、均匀随机放置的方法](#-二均匀随机放置的方法)
- [三、模型假设](#-三模型假设)
- [四、数学计算步骤](#-四数学计算步骤)
- [五、示例计算](#-五示例计算)
- [六、批量计算逻辑](#-六批量计算逻辑)
...
> [!NOTE]
> 坚持实践是检验真理的唯一标准，就是坚持马克思主义，坚持辩证唯物主义。——《光明日报》1978-05-11 特约评论员文章《实践是检验真理的唯一标准》
...
3. 直角坐标：
   $$
   x = 0 + 8 \times \cos(2.2619) \approx 8 \times (-0.637) = -5.1\ \text{cm} \\
   y = 0 + 8 \times \sin(2.2619) \approx 8 \times 0.771 = 6.2\ \text{cm}
   $$
   即这颗花生的位置为 $(-5.1, 6.2)\ \text{cm}$。
